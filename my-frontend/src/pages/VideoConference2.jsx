...
const VideoConference = () => {
  ...
  const mediaElementSourceNodes = useRef(new Map());
  ...
  const handleVideoAudioStream = (videoElement) => {
    if (!mediaElementSourceNodes.current.has(videoElement)) {
      ...
      detectVideoVolume();
      mediaElementSourceNodes.current.set(videoElement, source);
    }
  };
            ...
            {videoSources.map((source, index) => (
              <>
                <video
                  ...
                  onEnded={() => { handleVideoEnded(index); setRespuesta(false); }}
                  onPlay={(e) => {
                    setRespuesta(true);
                    setVideoEnded((prev) => {
                      const newVideoEnded = [...prev];
              ...
              <span className="flex items-center space-x-2">
                {!isSpeaking && (
                  <button
                    ...
                    className="relative w-32 h-12 bg-gray-300"
                  >
                    <div
                      className="absolute top-0 left-0 h-full bg-blue-500"
                      style={{ width: `${(timeLeft / countdownTime) * 100}%` }}
                    ></div>
                  ...
                  <WaveVisualizer
                    className={isSpeaking ? 'w-32 h-12' : 'w-32 h-12 hidden'}
                    onSpeakingChange={handleSpeakingChange}
                    silenceTimeout={2000} // Puedes ajustar el tiempo de silencio aquí
                  />
